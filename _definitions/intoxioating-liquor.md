---
title: Intoxioating Liquor
permalink: "/definitions/intoxioating-liquor.html"
body: Any liquor used as a beverage, and which, when so used in sufficient quantities,
  ordinarily or com-monly produces entire of partial intoxica-tion; any liquor intended
  for use as a bev-erage or capable of being so used, which contains alcohol, either
  obtained by fermen-tatiou or by the additional process of dis-tillatlon, iu such
  proportion that it will produce intoxication when imbibed ln such quan-tlties as
  may practlcally be drunk. Intox
published_at: '2018-07-07'
layout: post
---